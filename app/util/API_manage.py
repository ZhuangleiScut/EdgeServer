...
class ApiClient:
    ...
    def get_instances_exist_list(self):
        """
        获取当前存在的实例列表
        """
        all_list = self.get_instances_list()['instances']
        exist_list = list()
        for i in all_list:
            if i['state'] == 0:
                exist_list.append(i)
    ...
    def get_instance_resource(self, iid):
        """
        获取某个实例的资源配额信息(CPU与内存、容器数量)
        """
        data = json.loads(self.get_instance_detail(iid)['config'][0]['param'])
        nodes = 0
        if 'nodes' in data:
            nodes = data['nodes']
        elif 'workerservers' in data and 'parameterservers' in data:
            nodes += data['workerservers']
            nodes += data['parameterservers']
        elif nodes == 0:
            nodes = 1
        return data['cpu'], data['memory'], nodes
